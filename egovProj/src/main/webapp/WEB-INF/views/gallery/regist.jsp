					...
					<div class="input-group">
						<div class="custom-file">
							<input type="file" class="custom-file-input" name="uploadFile"
								id="input_imgs" multiple disabled /> <label class="custom-file-label"
								for="exampleInputFile">Choose file</label>
						</div>
						<div class="input-group-append">
							<span class="input-group-text swalDefaultSuccess" id="uploadBtn" style="cursor:pointer;">Upload</span>
						</div>
					</div>
				</div>
	...
	function fn_go(geta) {
		...
		$.each(bookVOList, function(index, item) {
			...
			if (str == bookId) {
				...
				//카테고리(category)/jquery를 이용한 selectbox select하기
				$("#category").val(item.category).prop("selected", true);
				//가격(price)
				$("#price").val(item.price);
				//입력 일자(insertDate)
				$("#insertDate").val(item.insertDate);
				//책 내용(에디터 내용 변경 <textarea id="content" name="content")
